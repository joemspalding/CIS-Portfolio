...
public class LibraryPatron
{
        ...
        set
        {
            if (!string.IsNullOrWhiteSpace(value)) // IsNullOrWhiteSpace includes tests for null, empty, or all whitespace
                ...
                throw new ArgumentOutOfRangeException($"{nameof(PatronName)}", value,
                    $"{nameof(PatronName)} {ValueIsEmptyErrorMessage()}");
        }
    }
        ...
        set
        {
            if (!string.IsNullOrWhiteSpace(value)) // IsNullOrWhiteSpace includes tests for null, empty, or all whitespace
                ...
                throw new ArgumentOutOfRangeException($"{nameof(PatronID)}", value,
                    $"{nameof(PatronID)} {ValueIsEmptyErrorMessage()}");
        }
    }
    #endregion
    ...
    private string ValueIsEmptyErrorMessage() => "must not be null or empty";

    // Precondition:  None
    // Postcondition: A string is returned presenting the libary patron's data on
    //                separate lines
    public override string ToString() => $"Name: {PatronName}, ID: {PatronID}";
    #endregion
